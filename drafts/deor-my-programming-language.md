...
<!-- themes: glacier -->
<!-- title: Deor — My Programming Language — Blog of Nathan Hoffman -->
...
One of the chief motivations behind Deor was to find a way to make Rust a more pleasant experience to write, a bit Python-like, with a bit of my own flare from many languages I love, while not actually creating an entirely new compiler (like Mojo), by transpiling directly to rust it gains the power of rust’s compiler and allows use of crates and raw Rust code in blocks. *Deor clones everything by default to make it easier to work with and avoid ownership (but copy types get uncloned by the compiler since they dont need it) Ownership still exists with the move keyword.*
...
I do want to say here that I respect rust immensely, it may be the low-level language to end all low level languages — esp. for those trying to do OS development — for example, it recently allowed me to use an alloc library to replace the std lib for a unikernel that had no base support for heap/vectors —insane! However, these I find are the exceptions to the rule — why not have both so that you can use whichever your application demands? If Deor transpiles to Rust, why can’t they live in harmony?
...
Structs are not reference types, they are treated as values: cloned, and are fully immutable. Furthermore, if you want to try to modify a struct you must create a new one. The good news is the “with” pattern makes this easy to construct a new struct using a base with new fields.
...
For functional patterns like lambdas, closures, etc. there is very little by intention — as this would overcomplicate things — rust blocks exist for that! The only pattern that survives is that a declared function can be passed as a first class member if it has one in and one output, this was added to still allow use popular first-class patterns like filter() / map(), etc. — but those are still up to you to create.
...
You will notice above there are no generics in a traditional sense, only symbols which are a bit restrictive (must be explicitly defined and have only 1 param : 1 output for funcs) this is another part of the Deor philosophy: only human-language not symbol-language. Just like when reading a book you will not find {} or <> you will not find them in Deor either. Parenthesis are used mainly to break up the word flow and are probably more common than both : and semicolons in texts — though the jury is out on that one — but it is such standard programming convention removing it would *decrease readability* — a cardinal sin in Deor.

As for typing in Deor I kept things simple, there are just a handful of types:

```
float e = 2.71828   # 64 bit
int three = 3       # 64 bit
bool isTrue = true
string hello_world = "Hello World"

# deor allows supports rust-like implicit assignment
pi as 3.14159265    # can be used for any type, so long as it is determinable
```

Anyway there is a ton more I could say about this, the documentation itself was first written by me but then overlaid so much by AI that I now somewhat hate the wording of it as the docs are now 90% AI generated, but it is about 99% correct at the moment, which you can find here if you are interested in more syntax: https://spec.deor.dev
